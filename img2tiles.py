...
# samples missing to download
tis_total = metadata.Tissue.value_counts()
tis_missed = metadata[~metadata.index.isin(samples_tiled)].Tissue.value_counts()
print(pd.concat([tis_total, tis_missed], axis=1))
# all samples
list(set(metadata.index) - set(samples_tiled))
missing_samples = list(set(metadata.index) - set(samples_tiled))

# by tissue
tis_total_bytissue = metadata[metadata["Tissue"]==tissue].index
tis_missed_bytissue = metadata[~metadata.index.isin(samples_tiled)][metadata["Tissue"]==tissue].index
print(tis_missed_bytissue)
